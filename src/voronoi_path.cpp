...
namespace voronoi_path
{
    ...
    bool voronoi_path::mapToGraph(Map map_)
    {
        ...
        // for (int i = 0; i < size; i += (pixels_to_skip + 1))
        // {
        //     //Occupied
        //     if (map.data[i] >= occupancy_threshold)
        //     {
        ...
        for (int i = 0; i < edge_vector.size(); i++)
        {
            ...
            for (int j = 0; j < hash_vec.size(); j++)
            {
                ...
                else
                {
                    ...
                    node_inf.push_back(GraphNode(edge_vector[i].pos[j].x, edge_vector[i].pos[j].y));
    ...
    std::string voronoi_path::hash(double x, double y)
    {
    ...
    std::vector<double> voronoi_path::dehash(std::string str)
    {
    ...
    std::vector<GraphNode> voronoi_path::getBezierPath(GraphNode point1, GraphNode point2, GraphNode point3)
    {
        std::vector<GraphNode> bezier_path;
        double dist1 = sqrt(pow(point1.x - point2.x, 2) + pow(point1.y - point2.y, 2));
        double dist2 = sqrt(pow(point3.x - point2.x, 2) + pow(point3.y - point2.y, 2));
        double total_dist = dist1 + dist2;

        double percent_interval = 1 / (total_dist / waypoint_sep);
        if (percent_interval > 1)
            percent_interval = 1;

        double curr_percent = 0;
        while (curr_percent <= 1.0)
        {
            GraphNode point12(point1.x * (1 - curr_percent) + (curr_percent)*point2.x, point1.y * (1 - curr_percent) + (curr_percent)*point2.y);
            GraphNode point23(point2.x * (1 - curr_percent) + (curr_percent)*point3.x, point2.y * (1 - curr_percent) + (curr_percent)*point3.y);
            GraphNode bezier_point(point12.x * (1 - curr_percent) + (curr_percent)*point23.x, point12.y * (1 - curr_percent) + (curr_percent)*point23.y);

            bezier_path.push_back(bezier_point);
            curr_percent += percent_interval;
        }
    ...
    std::vector<std::vector<GraphNode>> voronoi_path::getPath(GraphNode start, GraphNode end, int num_paths)
    {
        ...
        if (findShortestPath(start_node, end_node, shortest_path, cost))
        {
            ...
            for (int j = 0; j < all_path_nodes.size(); j++)
            {
                ...
                int i;
                int number_of_nodes = all_path_nodes[j].size();

                //For all nodes in path, i = node number in path j
                for (i = 0; i < number_of_nodes - 2; i += 2)
                {
                    std::vector<GraphNode> bezier_path = getBezierPath(all_path_nodes[j][i], all_path_nodes[j][i + 1], all_path_nodes[j][i + 2]);

                    if (!temp_path.empty())
                    {
                        temp_path.pop_back();
                        temp_path.insert(temp_path.end(), bezier_path.begin(), bezier_path.end());
                    }
                    else
                        temp_path.insert(temp_path.begin(), bezier_path.begin(), bezier_path.end());
                }

                //In case of leftover nodes
                if (i != number_of_nodes)
                {
                    while (i < number_of_nodes)
                    {
                        temp_path.push_back(all_path_nodes[j][i]);
                        i++;
                    }
                }
            ...
            // {
            //     std::vector<GraphNode> temp_path;
            //     // for (int i = 0; i < all_paths[j].size(); i++)
            //     int number_of_nodes = all_paths[j].size();

            //     for (int i = 1; i < number_of_nodes - 2; i += 2)
            //     {
            //         GraphNode curr_node(node_inf[all_paths[j][i]].x, node_inf[all_paths[j][i]].y);
            //         GraphNode int_node(node_inf[all_paths[j][i + 1]].x, node_inf[all_paths[j][i + 1]].y);
            //         GraphNode final_node(node_inf[all_paths[j][i + 2]].x, node_inf[all_paths[j][i + 2]].y);

            //         std::vector<GraphNode> bezier_path = getBezierPath(curr_node, int_node, final_node);
            ...
            //         else
            //             temp_path.insert(temp_path.begin(), bezier_path.begin(), bezier_path.end());
            //         // temp_path.push_back(GraphNode(node_inf[all_paths[j][i]].x, node_inf[all_paths[j][i]].y));
            //     }
    ...
    bool voronoi_path::getNearestNode(GraphNode start, GraphNode end, int &start_node, int &end_node)
    {
        auto start_time = std::chrono::system_clock::now();
    ...
    bool voronoi_path::kthShortestPaths(int start_node, int end_node, std::vector<int> shortestPath, std::vector<std::vector<int>> &all_paths, const int num_paths)
    {
        if (num_paths == 0)
        {
        ...
        for (int k = 1; k <= num_paths; k++)
        {
            ...
            for (int i = 0; i < kthPaths[k - 1].size() - 2; i++)
            {
                ...
                for (int i = 0; i < adj_list_removed_edges.size(); i++)
                {
                    adj_list[adj_list_removed_edges[i]] = adj_list_backup[adj_list_removed_edges[i]];
                }
    ...
    bool voronoi_path::findShortestPath(int start_node, int end_node, std::vector<int> &path, double &cost)
    {
        ...
        std::vector<int> nodes_prev(adj_list.size(), -1);
        ...
        open_list.emplace_back(std::pair<int, NodeInfo>(start_node, start_info));
        ...
        while (curr_node != end_node)
        {
            ...
            for (int i = 0; i < adj_list[curr_node].size(); i++)
            {
                ...
                if (it == open_list.end())
                {
                    ...
                    open_list.emplace_back(std::pair<int, NodeInfo>(next_node, new_node));
                }
            ...
            closed_list.emplace_back(std::pair<int, NodeInfo>(curr_node, curr_node_info));
    ...
    void voronoi_path::removeObstacleVertices()
    {
        ...
        //TODO: Find more memory efficient way to get remaining edges
        //Remove edges that are in obstacle
        //delete_indices is in ascending order
        //Copy undeleted edges to new container
    ...
    double voronoi_path::vectorAngle(double vec1[2], double vec2[2])
    {
    ...
    bool voronoi_path::edgeCollides(GraphNode start, GraphNode end)
    {
        double steps = 0;
    ...
    double voronoi_path::manhattanDist(GraphNode a, GraphNode b)
    {
        return fabs(a.x - b.x) + fabs(a.y - b.y);
    }

    double voronoi_path::euclideanDist(GraphNode a, GraphNode b)
    {
